...
::: {style="font-size: 55%;"}
**AI** = Insieme di tecniche e metodi che svolgono compiti tipici dell'intelligenza umana (ragionamento, apprendimento, comprensione del linguaggio naturale, ecc.)

**ML** = Sottoinsieme di modelli IA "allenati" (senza istruzioni esplicite) a riconoscere strutture nei dati / fare previsioni / raccomandare decisioni

<!-- viene meno la distinzione tra programmazione tradizionale e apprendimento automatico -->

**DL** =  i modelli DL apprendono a mappare input a output attraverso molteplici strati di parametri (reti neurali profonde) che estraggono caratteristiche complesse dai dati
<!-- Questo —trati successivi che estraggono caratteristiche e un algoritmo che aggiusta i pesi con backpropagation + gradient descent — è la base di tutta l’IA neurale moderna, sia:  DISCRIMINATIVA CHE GENERATIVA (se aggiungi un livello di decodifica che produce ...) -->

**Gen AI** = Invece che rispondere agli input con output predeterminati, i modelli generativi creano nuovi contenuti (testo, immagini, audio, video) simili ai dati su cui sono stati addestrati
...
:::: {style="font-size: 55%;"} 
`Strati` successivi analizzano dati in ingresso (e.g. pixel, parole) tramite un insieme di `neuroni` (i.e. mini modelli predittivi). Ogni strato produce un **output intermedio**, che diventa **input** dello strato successivo. L'algoritmo confronta l'**output finale** con esempi noti e "aggiusta" i pesi dei neuroni per minimizzare l'errore. 
